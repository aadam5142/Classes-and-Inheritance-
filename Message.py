# -*- coding: utf-8 -*-
"""
Created on Wed Nov 25 18:10:19 2020
...
#ABCDEFGHIJKLMNOPQRSTUVWXYZ
#A reminder from the introduction page - characters such as the space character, commas, periods, exclamation points, etc will not be encrypted by
# this cipher - basically, all the characters within string.punctuation, plus the space (' ') and all numerical characters (0 - 9) found in string.digits.

#Fill in the apply_shift(self, shift) method of the Message class. You may find it easier to use build_shift_dict(self, shift). Remember that spaces and punctuation 
...
class Message(object):
    def _init_(self, text):
        ...
        a Message object has two attribute:
    ...
    shift_lower_vales = lower_values[shift:] + lower_values[:shift]
    ...
    self.shift_ict = dict(zip(full_keys, full_values))
...
def apply_shift(self, shift):
    ...
    for i in self.message_text:
        if inot in self.build_shift_dict(shift).keys():
            new_msg.append(i)
            continue
        else:
